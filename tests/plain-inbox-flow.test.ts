/**
 * End-to-end tests for the plain (non-encrypted) inbox workflow.
 *
 * These tests simulate a real-world scenario by:
 * 1. Creating a plain inbox (with encryption: 'plain').
 * 2. Sending an email to that inbox via an SMTP client.
 * 3. Using the VaultSandboxClient to wait for, receive, and decode the email.
 * 4. Verifying the email's content, headers, and attachments.
 *
 * This test suite mirrors email-flow.test.ts but for plain (unencrypted) inboxes.
 *
...
describeIntegration('Plain Inbox Flow Integration Tests', () => {
  ...
  describe('Basic Plain Inbox Email Flow', () => {
    it('should create plain inbox and receive email', async () => {
      // Create a plain (non-encrypted) inbox
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);

      // Verify inbox is created with encryption disabled
      expect(inbox.encrypted).toBe(false);
      expect(inbox.emailAddress).toBeDefined();

      const testSubject = `Plain Test Email ${Date.now()}`;
      const testBody = 'This is a plain (non-encrypted) test email!';
    ...
    it('should handle HTML emails in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);

      expect(inbox.encrypted).toBe(false);

      const testSubject = `Plain HTML Test ${Date.now()}`;
    ...
    it('should extract links from plain inbox email', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
  ...
  describe('Plain Inbox Email Filtering', () => {
    it('should filter plain inbox emails by subject', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
    ...
    it('should filter plain inbox emails by sender', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
    ...
    it('should use custom predicate for filtering in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
  ...
  describe('Plain Inbox Multiple Emails', () => {
    it('should list all received emails in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
    ...
    it('should get specific email by ID in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);

      const testSubject = `Specific Plain Email ${Date.now()}`;
    ...
    it('should delete specific email in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
  ...
  describe('Plain Inbox Email Management', () => {
    it('should mark email as read in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
    ...
    it('should mark email as read via Email.markAsRead in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
    ...
    it('should delete email via Email.delete method in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
  ...
  describe('Plain Inbox Headers and Metadata', () => {
    it('should access email headers in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
    ...
    it('should have valid metadata in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
  ...
  describe('Plain Inbox Auth Results', () => {
    it('should have auth results structure in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
  ...
  describe('Plain Inbox Raw Email Access', () => {
    it('should get raw email content via Inbox.getRawEmail in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);

      const testSubject = `Raw Plain Email Test ${Date.now()}`;
    ...
    it('should get raw email content via Email.getRaw in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
  ...
  describe('Plain Inbox Export and Sync', () => {
    it('should export plain inbox without key material', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
    ...
    it('should update sync status when emails arrive in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
  ...
  describe('Plain Inbox Timeout Handling', () => {
    it('should timeout when email never arrives in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
    ...
    it('should timeout when waiting for non-matching email in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
  ...
  describe('Plain Inbox Real-World Scenarios', () => {
    it('should handle password reset email flow in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
    ...
    it('should handle welcome email with multiple links in plain inbox', async () => {
      const inbox = await client.createInbox({ encryption: 'plain' });
      createdInboxes.push(inbox);
